...
namespace ecs {
	...
	class Entity {
		// Manager puede acceder a las propiedades/métodos privados de Entitty
		friend Manager;
	...
	private:
		...
		// Por ejemplo, el componente físico est?en todas las entidades en la primera posición
	...
	public:
		...
		Entity(ecs::grpId_type gId) : gId_(gId), mngr_(nullptr), cmps_(), currCmps_(), alive_(), active_() {
		...
		// consultar si la entidad est?viva o no
		// que una entidad no est?viva significa que se puede cuando se haya recorrido
