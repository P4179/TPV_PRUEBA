...
SDLUtils::SDLUtils(std::string windowTitle, int width, int height) :
		...
		constsAccessWrapper_(consts_, "Consts Table") ///
{
...
SDLUtils::SDLUtils(std::string windowTitle, int width, int height,
		std::string filename, std::string constsFilename) :
		SDLUtils(windowTitle, width, height) {
	loadReasources(filename);
	loadConsts(constsFilename);
}
...
void SDLUtils::loadConsts(std::string filename) {
	// cargar el archivo JSON
	std::unique_ptr<JSONValue> jValueRoot(JSON::ParseFromFile(filename));
	// comprobar que se ha cargado correctamente y que root es un JSON object
	if (jValueRoot == nullptr || !jValueRoot->IsObject()) {
		throw "Something went wrong while load/parsing '" + filename + "'";
	}
	...
	if (jValue != nullptr) {
		...
		if (jValue->IsArray()) {
			// se reserva suficiente espacio para evitar que se sobredimensione
			consts_.reserve(jValue->AsArray().size());
			for (auto& v : jValue->AsArray()) {

				if (v->IsObject()) {
					JSONObject vObj = v->AsObject();
					std::string key = vObj["id"]->AsString();
					double number = vObj["number"]->AsNumber();
#ifdef _DEBUG
					std::cout << "Loading const with id: " << key << std::endl;
#endif
					consts_.emplace(key, number);
				}
				else {
					throw "'consts' array in '" + filename
						+ "' includes and invalid value";
				}
			}
		}
		else {
			throw "'consts' is not an array";
		}
	}
}
